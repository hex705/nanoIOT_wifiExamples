/*
 * 
 *  run this with sendLEDControl on another board. 
 *  config an instance on shiftr or use public credentails
 *  
 *  this file receives shiftr broker messages converst to 0,1 and writes to led 
 * 
 */
...
void onMqttMessage(int messageSize) {
  ...
  while (mqttClient.available()) { 
    ...
    LED_STATE = mqttClient.read(); // read teh char -- we know we are only sending one at a time
    LED_STATE = LED_STATE - 48; // need to get ascii into an int 

    digitalWrite(LED_BUILTIN, LED_STATE); // write the int to the LED
